...
"""
    ...
    Copyright (C) 2021 plugin.video.tubed.api
...
class CalculateN:
    ...
    def calculate_n(self, initial_n):
        ...
        if self.calculated_n:
            LOG.debug('`n` already calculated: {calculated_n}. returning early...'
        ...
        LOG.debug('Attempting to calculate `n` from initial: {initial_n}'
        ...
        for step in self.throttling_plan:
            ...
            if not callable(curr_func):
                LOG.debug('{curr_func} is not callable.'.format(curr_func=curr_func))
                LOG.debug('Throttling array:\n{throttling_array}\n'
        ...
        LOG.debug('Calculated `n`: {calculated_n}'
                  .format(calculated_n=self.calculated_n))
        return self.calculated_n
...
def find_object_from_startpoint(html, start_point):
    ...
    if html[0] not in ['{', '[']:
        LOG.debug('Invalid start point. Start of HTML:\n{html_start}'
...
def get_throttling_function_name(js):
    ...
    LOG.debug('Finding throttling function name')
    for pattern in function_patterns:
        regex = re.compile(pattern)
        function_match = regex.search(js)
        if function_match:
            LOG.debug("finished regex search, matched: %s" % pattern)
